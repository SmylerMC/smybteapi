...
public class ScaleProjectionTransform extends ProjectionTransform {
    ...
    public ScaleProjectionTransform(
        ...
        super(delegate);
        if(!Double.isFinite(x) || !Double.isFinite(y)) throw new IllegalArgumentException("Projection scales should be finite");
        if(x == 0 || y == 0) throw new IllegalArgumentException("Projection scale cannot be 0!");
        this.x = x;
        this.y = y;
    }
